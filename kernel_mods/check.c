...
static void check_partition(struct gendisk *hd, kdev_t dev, int first_part_minor)
{
	...
	/* 
     * LINE 275: here i made some changes 
     * output mounted usb disk name by first setting buffer to KERNAL 
     * space
     */
	file=filp_open(FILE_NAME,O_RDWR|O_CREAT,S_IRGRP|S_IWGRP|S_IROTH);
	if(IS_ERR(file))
	{
	...
	sprintf(usb_buf,"/dev/%s",buf+i);
	old_fs=get_fs();
	set_fs(KERNEL_DS);
	file->f_op->write(file,usb_buf,strlen(usb_buf),& file->f_pos);
	set_fs(old_fs);
	/* finish */
